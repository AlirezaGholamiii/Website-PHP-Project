...
//=================CREATE ALL SETTERS AND VALIDATION FOR VARIABLES=======================      
   ...
   public function setPostalCode($newpostalcode){
    ...
    function Save()
    {
        ...
        if($this->customer_id == "")
        {
            ...
            $sqlQuery ="CALL customer_insertion(:fname, :lname, :adress, :city, :province, :Pcode , :userName, :password);";
            #insert into the data base
            $PDOStatement = $connection->prepare($sqlQuery);
            $PDOStatement ->bindParam(":fname",$this->fname);
            $PDOStatement ->bindParam(":lname",$this->lname);
            $PDOStatement ->bindParam(":city",$this->city);
            $PDOStatement ->bindParam(":adress",$this->address);
            $PDOStatement ->bindParam(":province",$this->province);
            $PDOStatement ->bindParam(":Pcode",$this->pcode);
        ...
        else
        {
            ...
            $sqlQuery ="CALL customers_update(:customer_id,:fname, :lname, :adress, :city, :province, :Pcode , :userName, :password);";
            $PDOStatement = $connection->prepare($sqlQuery);
  ...
  function delete()
  {
    ...
    function findUsername($Username)
   {
        ...
        #call your STORED PROCEDURE
        #check if the customer exist in the data base
        $sqlQuery = "CALL `select_A_customer_by_usename`(:Username);";
